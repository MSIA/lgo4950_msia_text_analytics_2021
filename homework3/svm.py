...
def evaluate_svm(tr_x, tr_y, te_x, te_y, penalty_type, reg_parameter):
    ''' times the running of a linear svm and computes performance metrics'''

    # Start timer
    start = datetime.datetime.now()

    # train model
    model = LinearSVC(random_state=123, penalty=penalty_type,
                      loss='squared_hinge', dual=False,
                      C=reg_parameter, max_iter=100).fit(tr_x, tr_y)
        ...
        for reg_par in regularization_parameter:
            for pen in penalty_used:
                if data == 'uni':
                    ...
                    f1_iter, f1_micro_iter = evaluate_svm(x_train_bow,
                                                          y_train, x_test_bow,
                                                          y_test,
                                                          pen, reg_par)
                else:
                    ...
                    prec_iter, prec_micro_iter, \
                    rec_iter, rec_micro_iter, \
                    f1_iter, f1_micro_iter = evaluate_svm(x_train_bow_uni_bi,
...
if __name__ == '__main__':
    ...
    best_svm = LinearSVC(random_state=123, penalty='l2',
                         loss='squared_hinge', dual=False,
                         C=0.1, max_iter=100).fit(bow_representation, final_subset['stars'])
    # Saving model
    with open('model_svm.pickle', 'wb') as f:
        pickle.dump(best_svm, f)
    print("- Best svm model saved in Pickle")
